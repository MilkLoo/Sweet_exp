...
class Sea_Attention(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        #       (B,C_qk,H,W)-->mean-->(B,C_qk,H)-->reshape-->(B,h,d,H)-->permute-->(B,h,H,d);
        #       C_qk=h*d, h:注意力头的个数；d:每个注意力头的通道数
        qrow = self.pos_emb_rowq(q.mean(-1)).reshape(B, self.num_heads, -1, H).permute(0, 1, 3,
                                                                                       2)
        # 通过平均池化压缩水平方向,并为垂直方向的空间位置添加位置嵌入: (B,C_qk,H,W)-->mean-->(B,C_qk,H)-->reshape-->(B,h,d,H)
        krow = self.pos_emb_rowk(k.mean(-1)).reshape(B, self.num_heads, -1,
                                                     H)
        # 通过平均池化压缩水平方向: (B,C_v,H,W)-->mean-->(B,C_v,H)-->reshape-->(B,h,d_v,H)-->permute-->(B,h,H,d_v);
        # C_v=h*d_v, h:注意力头的个数；d_v:Value矩阵中每个注意力头的通道数
        ...
        # 对注意力机制的输出进行reshape操作,并进行卷积：(B,h,H,d_v)-->permute-->(B,h,d_v,H)-->reshape-->(B,C_v,H,1);   C_v=h*d_v
        xx_row = self.proj_encode_row(xx_row.permute(0, 1, 3, 2).reshape(B, self.dh, H,
                                                                         1))

        # squeeze column
        # 通过平均池化压缩垂直方向,并为水平方向的空间位置添加位置嵌入:
        #   (B,C_qk,H,W)-->mean-->(B,C_qk,W)-->reshape-->(B,h,d,W)-->permute-->(B,h,W,d);
        #   C_qk=h*d, h:注意力头的个数；d:每个注意力头的通道数
        qcolumn = self.pos_emb_columnq(q.mean(-2)).reshape(B, self.num_heads, -1, W).permute(0, 1, 3,
                                                                                             2)
        # 通过平均池化压缩垂直方向,并为水平方向的空间位置添加位置嵌入: (B,C_qk,H,W)-->mean-->(B,C_qk,W)-->reshape-->(B,h,d,W)
        ...
        # 对注意力机制的输出进行reshape操作,并进行卷积：(B,h,W,d_v)-->permute-->(B,h,d_v,W)-->reshape-->(B,C_v,1,W);  C_v=h*d_v
