...
class PW3D(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.openpose_joints_name = (
            ...
            'R_Ankle', 'L_Hip', 'L_Knee', 'L_Ankle', 'R_Eye', 'L_Eye', 'R_Ear', 'L_Ear', 'Pelvis')
        self.conf_thr = 0.05
        ...
        if cfg.pw3d:
            ...
            print("3dpw_crowd data len: ", len(self.datalist))
        elif cfg.pw3d_oc:
            print("3dpw_oc data len: ", len(self.datalist))
        elif cfg.pw3d_pc:
            print("3dpw_pc data len: ", len(self.datalist))
        ...
        for aid in db.anns.keys():
            ...
            if cfg.crowd and self.data_split == 'validation':
                try:
                    ...
                    hhrnetpose = openpose
                    hhrnetpose = transform_joint_to_other_db(hhrnetpose, self.openpose_joints_name,
                                                             self.coco_joints_name)
    ...
    def get_smpl_coord(self, smpl_param):
        ...
        smpl_trans = torch.FloatTensor(trans).view(-1,
                                                   3)
        # translation vector from smpl coordinate to 3dpw camera coordinate
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            pose_gt_img = transform_joint_to_other_db(pose_gt_img, self.h36m_joints_name, self.smpl.graph_joint_name)
            ...
            # pose_out_img[:, 2] =
            #   (pose_out_img[:, 2] / cfg.output_hm_shape[0] * 2. - 1) * (cfg.bbox_3d_size / 2) + root_h36m_gt[None, 2]
            # pose_out_cam = pixel2cam(pose_out_img, annot['cam_param']['focal'], annot['cam_param']['princpt'])
            # pose_coord_out_h36m =
            #   transform_joint_to_other_db(pose_out_cam, self.smpl.graph_joints_name, self.h36m_joints_name)
            ...
            # # debug
            # pose_out_img = cam2pixel(pose_coord_out_h36m + root_h36m_gt, annot['cam_param']['focal'],
            #                           annot['cam_param']['princpt'])
            # pose_out_img = transform_joint_to_other_db(pose_out_img, self.h36m_joints_name,
            #                                           self.smpl.graph_joints_name)
            ...
            eval_result['mpjpe'].append(np.sqrt(
                np.sum((pose_coord_out_h36m - pose_coord_gt_h36m) ** 2, 1)).mean() * 1000)  # meter -> millimeter
            ...
            if cfg.vis:
                ...
                #                                       kp_thresh=self.openpose_thr, alpha=1, kps_scores=pose_scores)
                ...
                newimg = vis_keypoints_with_skeleton(img.copy(), pose_out_img.T, self.smpl.graph_skeleton,
                                                     kp_thresh=0.4, alpha=1, kps_scores=pose_scores)
                newimg = vis_bbox(newimg, bbox_to_vis, alpha=1)
    ...
    def print_eval_result(eval_result):
        ...
        elif cfg.pw3d and cfg.fix_error:
