...
from common.utils.preprocessing import load_img, get_bbox, process_bbox, generate_patch_image, augmentation, compute_iou
from common.utils.transforms import world2cam, cam2pixel, pixel2cam, rigid_align, transform_joint_to_other_db
from common.utils.vis import vis_keypoints, vis_mesh, save_obj, vis_keypoints_with_skeleton, vis_bbox
...
class MuCo(torch.utils.data.Dataset):
    ...
    def get_fitting_error(self, muco_joint, smpl_mesh, do_flip):
        ...
        if do_flip:
            ...
            for pair in self.muco_flip_pairs:
                muco_joint[pair[0], :], muco_joint[pair[1], :] = muco_joint[pair[1], :].copy(), muco_joint[pair[0],
                                                                                                :].copy()
    ...
    def __getitem__(self, idx):
        ...
        if do_flip:
            ...
            for pair in self.muco_flip_pairs:
                muco_joint_img[pair[0], :], muco_joint_img[pair[1], :] = muco_joint_img[pair[1],
                                                                         :].copy(), muco_joint_img[pair[0], :].copy()
                muco_joint_cam[pair[0], :], muco_joint_cam[pair[1], :] = muco_joint_cam[pair[1],
                                                                         :].copy(), muco_joint_cam[pair[0], :].copy()
                muco_joint_valid[pair[0], :], muco_joint_valid[pair[1], :] = muco_joint_valid[pair[1],
                                                                             :].copy(), muco_joint_valid[pair[0],
                                                                                        :].copy()
        ...
        muco_joint_img[:, 2] = (muco_joint_img[:, 2] / (cfg.bbox_3d_size * 1000 / 2) + 1) / 2. * cfg.output_hm_shape[
            0]  # change cfg.bbox_3d_size from meter to millimeter

        # check truncation
        muco_joint_trunc = muco_joint_valid * (
                (muco_joint_img[:, 0] >= 0) * (muco_joint_img[:, 0] < cfg.output_hm_shape[2]) * \
                (muco_joint_img[:, 1] >= 0) * (muco_joint_img[:, 1] < cfg.output_hm_shape[1]) * \
                (muco_joint_img[:, 2] >= 0) * (muco_joint_img[:, 2] < cfg.output_hm_shape[0])).reshape(-1,
                                                                                                       1).astype(
            np.float32)
        ...
        input_muco_joint_img = transform_joint_to_other_db(input_muco_joint_img, self.muco_joints_name,
                                                           self.joints_name)

        if smpl_param is not None:
            # smpl coordinates
            smpl_mesh_cam, smpl_joint_cam, smpl_pose, smpl_shape = self.get_smpl_coord(smpl_param, cam_param, do_flip,
                                                                                       img_shape)
        ...
        # muco coordinate
        muco_joint_cam = np.dot(rot_aug_mat, muco_joint_cam.transpose(1, 0)).transpose(1,
                                                                                       0) / 1000  # millimeter to meter
        # parameter
        ...
        targets = {'orig_joint_img': muco_joint_img, 'fit_joint_img': smpl_joint_img, 'orig_joint_cam': muco_joint_cam,
                   'fit_joint_cam': smpl_joint_cam, 'pose_param': smpl_pose, 'shape_param': smpl_shape}
        meta_info = {'orig_joint_valid': muco_joint_valid, 'orig_joint_trunc': muco_joint_trunc,
