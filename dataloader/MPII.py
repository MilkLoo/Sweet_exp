...
from common.utils.preprocessing import compute_iou, process_bbox, load_img, augmentation
from common.utils.smpl import SMPL
from common.utils.transforms import transform_joint_to_other_db, cam2pixel


# from common.utils.vis import vis_keypoints_with_skeleton
...
class MPII(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.mpii_joints_name = (
            'R_Ankle', 'R_Knee', 'R_Hip', 'L_Hip', 'L_Knee', 'L_Ankle', 'Pelvis', 'Thorax', 'Neck', 'Head_top',
            'R_Wrist',
            'R_Elbow', 'R_Shoulder', 'L_Shoulder', 'L_Elbow', 'L_Wrist')
        self.mpii_flip_pairs = ((0, 5), (1, 4), (2, 3), (10, 15), (11, 14), (12, 13))
        self.mpii_skeleton = (
            (0, 1), (1, 2), (2, 6), (3, 6), (3, 4), (4, 5), (6, 7), (7, 8), (8, 9), (10, 11), (11, 12), (7, 12),
            (7, 13),
            (13, 14), (14, 15))
        ...
        self.coco_joints_name = (
            ...
            'R_Wrist', 'L_Hip', 'R_Hip', 'L_Knee', 'R_Knee', 'L_Ankle', 'R_Ankle', 'Pelvis')
            ...
            for aid in aids:
                ...
                tmplist.append(
                    {'img_path': img_path, 'img_shape': (height, width), 'bbox': bbox, 'tight_bbox': tight_bbox,
                     'joint_img': joint_img, 'joint_valid': joint_valid, 'smpl_param': smpl_param})
            ...
            for i, person in enumerate(tmplist):
                ...
                for other in other_persons:
                    ...
                    if iou < 0.1:
                        continue
                    num_overlap += 1
                    other_joint = transform_joint_to_other_db(other['joint_img'], self.mpii_joints_name,
                                                              self.coco_joints_name)
        ...
        if do_flip:
            ...
            for pair in self.flip_pairs:
                if pair[0] < len(smpl_pose) and pair[1] < len(
                        smpl_pose):  # face keypoints are already included in self.flip_pairs. However,
                    # they are not included in smpl_pose.
    ...
    def __getitem__(self, idx):
        ...
        if do_flip:
            ...
            for pair in self.mpii_flip_pairs:
                mpii_joint_img[pair[0], :], mpii_joint_img[pair[1], :] = mpii_joint_img[pair[1],
                                                                         :].copy(), mpii_joint_img[pair[0], :].copy()
        ...
        mpii_joint_trunc = mpii_joint_valid * (
                (mpii_joint_img[:, 0] >= 0) * (mpii_joint_img[:, 0] < cfg.output_hm_shape[2]) * (
                mpii_joint_img[:, 1] >= 0) * (mpii_joint_img[:, 1] < cfg.output_hm_shape[1])).reshape(-1,
                                                                                                      1).astype(
            np.float32)
        ...
        input_mpii_joint_img = transform_joint_to_other_db(input_mpii_joint_img, self.mpii_joints_name,
                                                           self.joints_name)
        ...
        targets = {'orig_joint_img': mpii_joint_img, 'fit_joint_img': smpl_joint_img, 'orig_joint_cam': mpii_joint_cam,
                   'fit_joint_cam': smpl_joint_cam, 'pose_param': smpl_pose, 'shape_param': smpl_shape}
        meta_info = {'orig_joint_valid': mpii_joint_valid, 'orig_joint_trunc': mpii_joint_trunc,
