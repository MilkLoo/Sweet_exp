...
from common.utils.preprocessing import load_img, process_bbox, augmentation, compute_iou
...
class CrowdPose(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.crowdpose_joints_name = (
            'L_Shoulder', 'R_Shoulder', 'L_Elbow', 'R_Elbow', 'L_Wrist', 'R_Wrist', 'L_Hip', 'R_Hip', 'L_Knee',
            'R_Knee',
            'L_Ankle', 'R_Ankle', 'Head_top', 'Neck', 'Pelvis')
        self.crowdpose_skeleton = (
            (0, 2), (0, 13), (1, 3), (1, 13), (2, 4), (3, 5), (6, 14), (7, 14), (6, 8), (7, 9), (8, 10), (9, 11),
            (12, 13),
            (13, 14))
            ...
            for split in split_list:
                ...
                for iid in db.imgs.keys():
                    ...
                    for aid in aids:
                        ...
                        tmplist.append(
                            {'img_path': img_path, 'img_shape': (height, width), 'bbox': bbox, 'tight_bbox': tight_bbox,
                             'joint_img': joint_img, 'joint_valid': joint_valid, 'neural_annot_result': smpl_param})
                    ...
                    for i, person in enumerate(tmplist):
                        ...
                        for other in other_persons:
                            ...
                            if iou < 0.1:
                                continue
                            num_overlap += 1
                            other_joint = transform_joint_to_other_db(other['joint_img'], self.crowdpose_joints_name,
    ...
    def get_smpl_coord(self, smpl_param, cam_param, do_flip, img_shape):
        ...
        if do_flip:
            ...
            for pair in self.flip_pairs:
                if pair[0] < len(smpl_pose) and pair[1] < len(
                        smpl_pose):  # face keypoints are already included in self.flip_pairs. However, they are not included in smpl_pose.
                    smpl_pose[pair[0], :], smpl_pose[pair[1], :] = smpl_pose[pair[1], :].clone(), smpl_pose[pair[0],
                                                                                                  :].clone()
        ...
        if do_flip:  # avg of old and new root joint should be image center.
            ...
            flip_trans_x = 2 * (((img_shape[1] - 1) / 2. - princpt[0]) / focal[0] * (
                smpl_joint_coord[self.root_joint_idx, 2])) - 2 * smpl_joint_coord[self.root_joint_idx][0]
        ...
        if self.data_split == 'train':
            ...
            if do_flip:
                ...
                for pair in self.crowdpose_flip_pairs:
                    crowdpose_joint_img[pair[0], :], crowdpose_joint_img[pair[1], :] = crowdpose_joint_img[pair[1],
                                                                                       :].copy(), crowdpose_joint_img[
                                                                                                  pair[0], :].copy()
                    crowdpose_joint_valid[pair[0], :], crowdpose_joint_valid[pair[1], :] = crowdpose_joint_valid[
                                                                                           pair[1],
                                                                                           :].copy(), crowdpose_joint_valid[
                                                                                                      pair[0], :].copy()

            crowdpose_joint_img_xy1 = np.concatenate(
                (crowdpose_joint_img[:, :2], np.ones_like(crowdpose_joint_img[:, :1])), 1)
            ...
            # check truncation
            crowdpose_joint_trunc = crowdpose_joint_valid * (
                    (crowdpose_joint_img[:, 0] >= 0) * (crowdpose_joint_img[:, 0] < cfg.output_hm_shape[2]) * (
                    crowdpose_joint_img[:, 1] >= 0) * (
                            crowdpose_joint_img[:, 1] < cfg.output_hm_shape[1])).reshape(
                -1, 1).astype(np.float32)

            # transform coco joints to target db joints
            crowdpose_joint_img = transform_joint_to_other_db(crowdpose_joint_img, self.crowdpose_joints_name,
                                                              self.joints_name)
            crowdpose_joint_cam = np.zeros((self.joint_num, 3), dtype=np.float32)  # dummy
            crowdpose_joint_valid = transform_joint_to_other_db(crowdpose_joint_valid, self.crowdpose_joints_name,
                                                                self.joints_name)
            crowdpose_joint_trunc = transform_joint_to_other_db(crowdpose_joint_trunc, self.crowdpose_joints_name,
                                                                self.joints_name)

            # apply PoseFix
            tmp_joint_img = transform_joint_to_other_db(input_crowdpose_joint_img, self.crowdpose_joints_name,
                                                        self.coco_joints_name)
            tmp_joint_img = replace_joint_img(tmp_joint_img, data['tight_bbox'], near_joints, data['num_overlap'],
                                              img2bb_trans)
            tmp_joint_img = transform_joint_to_other_db(tmp_joint_img, self.coco_joints_name,
                                                        self.crowdpose_joints_name)
            input_crowdpose_joint_img[self.crowdpose_coco_common_jidx, :2] = tmp_joint_img[
                                                                             self.crowdpose_coco_common_jidx, :2]
            """
            ...
            """
            input_crowdpose_joint_img[:, 0] = input_crowdpose_joint_img[:, 0] / cfg.input_img_shape[1] * \
                                              cfg.output_hm_shape[2]
            input_crowdpose_joint_img[:, 1] = input_crowdpose_joint_img[:, 1] / cfg.input_img_shape[0] * \
                                              cfg.output_hm_shape[1]
            input_crowdpose_joint_img = transform_joint_to_other_db(input_crowdpose_joint_img,
                                                                    self.crowdpose_joints_name, self.joints_name)
            ...
            targets = {'orig_joint_img': crowdpose_joint_img, 'fit_joint_img': smpl_joint_img,
                       'orig_joint_cam': crowdpose_joint_cam, 'fit_joint_cam': smpl_joint_cam, 'pose_param': smpl_pose,
                       'shape_param': smpl_shape}
            meta_info = {'orig_joint_valid': crowdpose_joint_valid, 'orig_joint_trunc': crowdpose_joint_trunc,
