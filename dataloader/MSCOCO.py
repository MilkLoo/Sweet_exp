...
from common.utils.vis import vis_bbox, render_mesh
...
class MSCOCO(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #     db_path = osp.join(cfg.root_dir, 'data', 'MSCOCO', 'images_lmdb', 'images_lmdb.lmdb')
        ...
        if self.data_split == 'train':
            ...
            coco_joint_trunc = coco_joint_valid * (
                    ...
                    (coco_joint_img[:, 1] >= 0) * (coco_joint_img[:, 1] < cfg.output_hm_shape[1])).reshape(-1,
                                                                                                           1).astype(
                np.float32)
            ...
            # apply PoseFix
            input_coco_joint_img = replace_joint_img(input_coco_joint_img, data['tight_bbox'], near_joints,
                                                     data['num_overlap'], img2bb_trans)
            """
            ...
            input_coco_joint_img = transform_joint_to_other_db(input_coco_joint_img, self.coco_joints_name,
                                                               self.joints_name)
