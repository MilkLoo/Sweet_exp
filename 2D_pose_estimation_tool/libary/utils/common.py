...
def draw_humans(npimg, humans, imgcopy=False):
    ...
    for human in humans:
        ...
        for i in range(CocoPart.Background.value):
            ...
            # center = (body_part.x * image_w + 0.5, body_part.y * image_h + 0.5)
            centers[i] = center
    ...
    for human in humans:
        for i in range(CocoPart.Background.value):
            if i not in human.body_parts.keys():
                continue
            body_part = human.body_parts[i]
            center = [body_part.x * image_w + 0.5, body_part.y * image_h + 0.5, body_part.score]
            # center = [int(body_part.x * image_w + 0.5), int(body_part.y * image_h + 0.5), body_part.score]
            centers[i] = center
        centers_deep = copy.deepcopy(centers)
        all_coord[human_num] = centers_deep
        human_num += 1
    return all_coord, human_num


def human_coord_trans(centers, human_num):
    new_pose_2d_data = {}
    for i in range(human_num):
        pose_2d_data = centers[i]
        for j in range(CocoPart.Background.value):
            if j not in pose_2d_data.keys():
                pose_2d_data[j] = [0.0, 0.0, 0.0]
            # 交换关键点内容
            if j == 1:
                pose_2d_data.pop(j)
        for key, value in pose_2d_data.items():
            if key == 0:
                new_pose_2d_data[key] = value
            else:
                new_pose_2d_data[key - 1] = value
        new_pose_2d_data_deep = copy.deepcopy(new_pose_2d_data)
        centers[i] = new_pose_2d_data_deep
    return centers


def trans_value(centers, human_num):
    coco_pose = {}
    for i in range(human_num):
        pose_2d_data = centers[i]
        for j in range(CocoPart.Background.value):
            if j == 0:
                coco_pose[0] = pose_2d_data[j]
            if j == 1:
                coco_pose[6] = pose_2d_data[j]
            if j == 2:
                coco_pose[8] = pose_2d_data[j]
            if j == 3:
                coco_pose[10] = pose_2d_data[j]
            if j == 4:
                coco_pose[5] = pose_2d_data[j]
            if j == 5:
                coco_pose[7] = pose_2d_data[j]
            if j == 6:
                coco_pose[9] = pose_2d_data[j]
            if j == 7:
                coco_pose[12] = pose_2d_data[j]
            if j == 8:
                coco_pose[14] = pose_2d_data[j]
            if j == 9:
                coco_pose[16] = pose_2d_data[j]
            if j == 10:
                coco_pose[11] = pose_2d_data[j]
            if j == 11:
                coco_pose[13] = pose_2d_data[j]
            if j == 12:
                coco_pose[15] = pose_2d_data[j]
            if j == 13:
                coco_pose[2] = pose_2d_data[j]
            if j == 14:
                coco_pose[1] = pose_2d_data[j]
            if j == 15:
                coco_pose[4] = pose_2d_data[j]
            if j == 16:
                coco_pose[3] = pose_2d_data[j]
        coco_pose_deep = copy.deepcopy(coco_pose)
        centers[i] = coco_pose_deep
    return centers


def get_coco_coord(centers, human_num):
    step_1 = human_coord_trans(centers, human_num)
    step_2 = trans_value(step_1, human_num)
    return step_2
...
# CocoPairs = [
#     (6, 17), (17, 5), (6, 8), (8, 10), (5, 7), (7, 9), (17, 12), (12, 14), (14, 16), (17, 11),
#     (11, 13), (13, 15), (17, 0), (0, 2), (2, 4), (0, 1), (1, 3), (6, 4), (5, 3)
# ]  # = 19
