...
def load_pascal_occluder(pascal_voc_root_path):
    """
     用于从PASCAL VOC数据集加载遮挡物
     功能:
        遍历PASCAL VOC数据集中的每个标注文件，找到包含人物标注的图像。
        从图像中提取人物的标注框，并根据标注框提取人物的掩码。
        将提取的人物掩码叠加到原始图像上，形成带有掩码的人物图像。
        对带有掩码的人物图像进行下采样处理，以提高处理效率。
        将处理后的遮挡物添加到遮挡物列表中。
    :param pascal_voc_root_path: PASCAL VOC数据集的根目录路径
    :return: 一个遮挡物列表，其中每个遮挡物是一个带有遮罩的图像，用于后续的图像处理和合成操作。
    """
    occluders = []
