...
from common.utils.human_body_prior.src.human_body_prior.body_model.lbs import lbs
import sys
...
class BodyModel(nn.Module):
    ...
    def __init__(self,
        ...
        assert self.model_type in ['smpl', 'smplh', 'smplx', 'mano', 'mano', 'animal_horse', 'animal_dog', 'flame',
                                   'animal_rat'], ValueError(
        ...
        self.comp_register('init_v_template', torch.tensor(smpl_dict['v_template'][None], dtype=dtype),
                           persistent=persistant_buffer)

        self.comp_register('f', torch.tensor(smpl_dict['f'].astype(np.int32), dtype=torch.int32),
                           persistent=persistant_buffer)
        ...
        self.comp_register('J_regressor', torch.tensor(smpl_dict['J_regressor'], dtype=dtype),
                           persistent=persistant_buffer)
        ...
        self.comp_register('kintree_table', torch.tensor(kintree_table, dtype=torch.int32),
                           persistent=persistant_buffer)
        ...
        if self.model_type in ['smpl']:
            ...
            self.comp_register('init_pose_hand', torch.zeros((1, 15 * 3 * 2), dtype=dtype),
                               persistent=persistant_buffer)
    ...
    def forward(self, root_orient=None, pose_body=None, pose_hand=None, pose_jaw=None, pose_eye=None, betas=None,
        ...
        for arg in [root_orient, pose_body, pose_hand, pose_jaw, pose_eye, betas, trans, dmpls, expression, v_template,
                    joints]:
            if arg is not None:
        ...
        assert self.model_type in ['smpl', 'smplh', 'smplx', 'mano', 'animal_horse', 'animal_dog', 'flame',
                                   'animal_rat'], ValueError(
            'model_type should be in smpl/smplh/smplx/mano')
        ...
        if self.model_type in ['smplh', 'smpl']:
            ...
            full_pose = torch.cat([root_orient, pose_body, pose_jaw, pose_eye],
                                  dim=-1)  # orient:3, body:63, jaw:3, eyel:3, eyer:3, handl, handr
        elif self.model_type in ['mano', ]:
