...
def intra_info_nce_loss(feature_space_3d, gt_joint_img, pred_joint_img, joint_mask, anchour_num=50, pos_num=100,
                        neg_num=200, temperature=0.1):
    """
    Args:
        feature_space_3d: (B, C, D, H, W)
        gt_joint_img: (B, J, 3)
    ...
    positive_key = []
    nagative_key = []
    sample_gt_joint_feat = F.grid_sample(feature_space_3d, gt_joint_img.unsqueeze(1).unsqueeze(1), mode='bilinear',
                                         align_corners=True).squeeze(2).squeeze(2).permute(0, 2,
                                                                                           1).contiguous()  # B, J, C
    sample_pred_joint_feat = F.grid_sample(feature_space_3d, pred_joint_img.unsqueeze(1).unsqueeze(1), mode='bilinear',
                                           align_corners=True).squeeze(2).squeeze(2).permute(0, 2,
                                                                                             1).contiguous()  # B, J, C
    for label in anchour_label:
        ...
        if torch.sum(label_mask) == 0:  # 没有正样本
            ...
            # sample positive set
            pos_idx = np.random.choice(np.where(label_mask.cpu().numpy())[0], pos_num,
                                       replace=torch.sum(label_mask).item() < pos_num)
    ...
    loss = info_nce(anchour_faet, positive_key, negative_keys=nagative_key, temperature=temperature, reduction='none',
...
def inter_info_nce_loss(feature_space_3d, pred_joint_img, anchour_num=50, pos_num=100, neg_num=200, temperature=0.1):
    ...
    sample_pred_joint_feat = F.grid_sample(feature_space_3d, pred_joint_img.unsqueeze(1).unsqueeze(1), mode='bilinear',
                                           align_corners=True).squeeze(2).squeeze(2).permute(0, 2,
                                                                                             1).contiguous()  # B, J, C
    pred_joint_img = pred_joint_img.add(1).mul(4)  # (0 ~ 8) (B, J, 3)    (-1,1) --> (0,8)
    ...
    loss = info_nce(anchour_feat, pos_faet, negative_keys=neg_feat, temperature=temperature, reduction='mean',
